...
export class DashboardComponent implements OnInit {
  ...
  sort(prop) {
    ...
    if (this.objForSorting.toggle) {
      const obj = this.dataForView.cars.sort((a, b) => this.compareCars(a, b, prop) );
      this.dataForView.cars = obj;
      this.objForSorting.CSSclass = 'fa-sort-asc';
    } else {
      const obj = this.dataForView.cars.sort((a, b) => this.compareCars(b, a, prop));
      this.dataForView.cars = obj;
  ...
  private compareCars(a, b, prop): number {
    if (a[prop] > b[prop]) {
      return -1;
    }
    if (a[prop] < b[prop]) {
